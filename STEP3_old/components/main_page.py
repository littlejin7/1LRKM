...
def _match(
    ...
    if "pos" in raw_sent or "긍" in raw_sent:
        item_sent_ko = "긍정"
    elif "neg" in raw_sent or "부" in raw_sent:
        item_sent_ko = "부정"
    else:
        item_sent_ko = "중립"
...
def render_ranking(filtered: list):
    ...
    if briefing_click:
        ...
        for i, item in enumerate(top5, 1):
            tags = _safe_tags(item.get("artist_tags", []))
...
def render_dashboard(
    ...
    print(f"DEBUG: ranking_items count = {len(ranking_items)}")
