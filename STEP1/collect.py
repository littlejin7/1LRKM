...
def extract_person_hint(title: str, content: str) -> str:
    ...
    # 1. 텍스트 준비 및 정규식 추출
    title = title or ""
    body_sample = content[:1500] if content else ""
    
    en_title = _EN_NAME_RE.findall(title)
    ko_title = _KO_NAME_RE.findall(title)
    
    en_body = _EN_NAME_RE.findall(body_sample)
    ko_body = _KO_NAME_RE.findall(body_sample)
    
    # 2. 서술어/수식어 어미 필터링용 정규식
    # (~한, ~된, ~던, ~고, ~서, ~기, ~며 등 아티스트 이름이 아닐 확률이 높은 어미)
    _JUNK_ENDINGS = re.compile(r".*([은는한된던고서기며며니라자요])$")
    
    def is_valid_hint(word: str) -> bool:
        if word in _KO_STOPWORDS: return False
        if word.isdigit(): return False
        if len(word) < 2: return False
        # 형용사/동사형 어미 필터링
        if _JUNK_ENDINGS.match(word): return False
        return True

    # 3. 가중치 부여 (제목 단어는 5점, 본문 단어는 1점)
    counts = collections.Counter()
    
    for w in en_title + ko_title:
        if is_valid_hint(w):
            counts[w] += 5
            
    for w in en_body + ko_body:
        if is_valid_hint(w):
            counts[w] += 1
                ...
                results.append(
                    {
                        ...
                        "category": category,
                        "sub_category": sub_category,
                        # ★ 신규: 인물 힌트 (raw_text HTML 대신 정제된 content 활용하여 정확도 향상)
                        "raw_artist_hint": extract_person_hint(title, content),
                    }
                )
            return results
